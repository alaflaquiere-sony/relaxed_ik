...
if info_file_name == "":
    print(
        bcolors.WARNING
        + "ERROR: info_file_name is a required field in load_info_file.launch.  Please fill in that parameter and run again."
        + bcolors.ENDC
    )
    exit(-1)
...
if not file_exists:
    print(
        bcolors.WARNING
        + "{} file could not be found in RelaxedIK/Config/info_files.  Was there a misspelling?".format(info_file_name)
        + bcolors.ENDC
    )
    exit(-1)
