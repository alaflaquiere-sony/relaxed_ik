...
def get_relaxedIK_vars_from_info_file(path_to_src, info_file_name="", preconfig=False):
    ...
    if not preconfig:
        vars = RelaxedIK_vars(
            ...
            collision_nn_file=collision_nn_file,
        )
    else:
        vars = RelaxedIK_vars(
            "robot",
...
def get_relaxedIK_from_info_file(
    path_to_src, info_file_name="", optimization_package="scipy", solver_name="slsqp", preconfig=False
):
