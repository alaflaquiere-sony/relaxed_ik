...
class RelaxedIK_vars(Vars):
    def __init__(
        ...
        objectives=(
            Position_MultiEE_Obj(),
            Orientation_MultiEE_Obj(),
            Min_Jt_Vel_Obj(),
            Min_Jt_Accel_Obj(),
            Min_Jt_Jerk_Obj(),
            Joint_Limit_Obj(),
            Collision_Avoidance_nn(),
        ),
        weight_funcs=(
            Identity_Weight(),
            Identity_Weight(),
        ...
        Vars.__init__(
            self, name, objective_function, init_state, objectives, weight_funcs, weight_priors, constraints, bounds
        )
        # check inputs #####################################################################################################################
        ...
        if not (num_objs == len(weight_funcs) == len(weight_priors)):
            print(
                bcolors.FAIL + "Invalid Inputs.  The number of objectives ({}) must be the same as the number"
                "of weight functions ({}) and weight priors ({}).  Exiting.".format(
                    str(num_objs), str(len(weight_funcs)), str(len(weight_priors))
                )
                + bcolors.ENDC
            )
        ...
        if not self.numDOF == len(init_state):
            ...
            print(
                bcolors.WARNING
                + "WARNING: Length of init_state does not match number of robot DOFs.  Is this what you intended?"
                + bcolors.ENDC
            )
    ...
    def update(self, xopt, f_obj, publish_objectives=True, publish_constraints=True, publish_weight_funcs=True):
        Vars.update(
            ...
            publish_weight_funcs=publish_weight_funcs,
        )
