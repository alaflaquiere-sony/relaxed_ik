...
class Config_Engine:
    ...
    def check_for_config_file(self):
        ...
        for f in files:
            ...
            if ext == "config":
                response = raw_input(
                    bcolors.OKGREEN
                    + "Found saved config file "
                    + f
                    + ".  Would you like to use this config file? (y or n): "
                    + bcolors.ENDC
                )
                if response == "y":
                    return f
                    ...
                    response = raw_input(
                        bcolors.OKGREEN + "Would you like to generate a new config file? (y or n): " + bcolors.ENDC
                    )
                    if response == "y":
                        return None
                    else:
                        print(
                            bcolors.FAIL
                            + "Exiting.  Please manually specify which config file you would like to use on the next run."
                            + bcolors.ENDC
                        )
                        exit(1)
    ...
    def generate_config_file(self):
        ...
        file_vars = [
            robot_name,
            collision_nn,
            self.vars.init_state,
            self.vars.full_joint_lists,
            self.vars.fixed_ee_joints,
            self.vars.joint_order,
            self.vars.urdf_path,
            self.vars.collision_file,
        ]
