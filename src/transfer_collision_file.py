...
def __get_box_string(collision_file_yaml, info_file_yaml, idx):
    ...
    p = box["parameters"]
    out_str += "    parameters: [{}, {}, {}]".format(p[0] / 2.0, p[1] / 2.0, p[2] / 2.0) + "\n"
    out_str += (
        "    coordinate_frame: {}".format(__get_coordinate_frame_string(info_file_yaml, box["coordinate_frame"])) + "\n"
    )
    r = box["rotation"]
...
def __get_sphere_string(collision_file_yaml, info_file_yaml, idx):
    ...
    out_str += (
        "    coordinate_frame: {}".format(__get_coordinate_frame_string(info_file_yaml, sphere["coordinate_frame"]))
        + "\n"
    )
    r = sphere["rotation"]
    out_str += "    rotation: [{}, {}, {}]".format(float(r[0]), float(r[1]), float(r[2])) + "\n"
